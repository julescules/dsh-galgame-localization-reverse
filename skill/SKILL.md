...
description: Windows galgame/视觉小说汉化、保护机制逆向与补丁工程工作流。Use for engine detection, third-party patch forensics, archive/encryption/license analysis, DMM/SoftDenchi launch-authorization bypasses, compatibility isolation, text/image/UI localization, encoding repair, save-resume compatibility, reversible packaging, portable releases, graphical installers, and bytecode tooling. Covers SILKY'S/AI6WIN, Director, KiriKiri, NScripter/ONScripter, Ren'Py, Unity, RealLive/AVG2000, SiglusEngine, Artemis, BGI/Ethornell/BURIKO, Yu-Ris, Majiro, CatSystem2, QLIE, WillPlus/AdvHD, Cmvs, Escude, Softpal, NeXAS, Malie, LiveMaker, RPG Maker, WolfRPG, TyranoScript, and SPT60/SBY/ASZB custom engines.
---
...
4. Normalize text through a UTF-8 intermediate format while preserving source path, source hash, original encoding, newline style, segment IDs, placeholder map, and engine metadata.
...
6. Run translation QA before writing back: placeholder preservation, line length, repeated-string consistency, terminology hits, encoding round-trip, font/UI capacity, and sampled in-game checks. Put every intentional untranslated string or asset in an explicit, hash-bound exclusion list; never hide unfinished work in the residual-text allowlist.
