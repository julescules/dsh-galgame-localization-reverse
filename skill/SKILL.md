...
5. When a game update, script rebuild, or extraction changes segment IDs, migrate prior translations with `scripts/vn_translation_memory.py`. It may auto-reuse only an exact segment-ID/source pair or an exact source match whose prior non-empty targets agree and whose placeholders remain valid. Treat conflicting prior targets and placeholder mismatches as blockers. Keep fuzzy matches in the review queue only; never auto-apply them. Write to new JSONL/report paths, preserve both inputs, and bind the report to both input SHA-256 hashes.
