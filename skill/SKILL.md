...
6. Run translation QA before writing back. Use `scripts/vn_qa.py` for generic JSON/JSONL or GalTransl cache records; its default is strict control-token parity, while completeness can be promoted to a release blocker. Review its JSON evidence and optional Markdown report for empty translations, residual Japanese, unchanged source text, encoding failures, line length, repeated-string consistency, terminology hits, and font/UI capacity. Put every intentional untranslated string or asset in an explicit, hash-bound exclusion list; never hide unfinished work in the residual-text allowlist.
